...
const compactSigned=(value,max=0)=>`${Number(value||0)>0?'+':Number(value||0)<0?'-':''}${compactNumber(Math.abs(Number(value||0)),max)}`;
const formatMoneyCompactCard=value=>`${Number(value||0)>=0?'+':''}$${compactNumber(Math.abs(Number(value||0)),0)}`;
const formatMoneyClean=value=>`${Number(value||0)<0?'-':''}$${compactNumber(Math.abs(Number(value||0)),0)}`;
const formatPercentCard=value=>`${compactNumber(Number(value||0),0)}%`;
const formatMetricCard=(value,max=0)=>compactNumber(value,max);
...
function useAccountFilter(trades=[],settings={}){
  const initial=()=>localStorage.getItem('mtc-active-account')||'__all__';
  const [active,setActive]=useState(initial);
  const accountOptions=useMemo(()=>Array.from(new Set([...normalizedAccounts(settings).map(a=>a.name),...(trades||[]).map(accountName)])).filter(Boolean).sort(), [trades,settings?.accounts,settings?.initialBalance]);
  const accounts=useMemo(()=>['__all__',...accountOptions], [accountOptions]);
  useEffect(()=>{if(active!=='__all__'&&!accountOptions.includes(active))setActive('__all__')},[accountOptions.join('|')]);
  useEffect(()=>{localStorage.setItem('mtc-active-account',active)},[active]);
  const filtered=useMemo(()=>active==='__all__'?(trades||[]):(trades||[]).filter(t=>accountName(t)===active),[trades,active]);
  return {active,setActive,accounts,filtered};
}
...
function calc(trades,initial=10000){
  ...
  const planFollowed=closed.filter(t=>t.followedPlan===true || (t.executionBehaviors||[]).includes('Seguí el plan'));
  const negativeBehaviorOptions=['Dudé antes de entrar','Entré tarde','Me anticipé','Moví el stop','Cerré antes de tiempo','Sobreoperé','Operé por impulso','Rompí reglas'];
...
function insights(s){
  ...
  cards.push({label:'Expectativa',value:s.count?money(s.expectancy):'—',tone:s.expectancy>0?'good':s.expectancy<0?'bad':'neutral',text:'Promedio real que entrega cada trade registrado.'});
  if(s.bySession.length){const b=[...s.bySession].sort((a,b)=>b.value-a.value)[0]; cards.push({label:'Mejor sesión',value:b.name,tone:b.value>=0?'good':'bad',text:`Resultado acumulado: ${money(b.value)} en ${b.count} trade(s).`});}
  if(s.byBias?.length){const b=s.byBias[0]; cards.push({label:'Sesgo más rentable',value:b.name,tone:b.value>=0?'good':'warn',text:`Ese sesgo generó ${money(b.value)} en ${b.count} trade(s). Úsalo para detectar dónde está tu ventaja.`});}
  if(s.bySetup.length){const b=s.bySetup[0]; cards.push({label:'Setup dominante',value:b.name,tone:b.value>=0?'good':'warn',text:`Resultado acumulado: ${money(b.value)}. Evita dispersarte fuera de tu edge.`});}
  cards.push({label:'Drawdown máximo',value:pct(s.maxDD),tone:s.maxDD<=5?'good':s.maxDD<=10?'warn':'bad',text:`Caída máxima estimada: ${money(s.ddMoney)} desde el pico de equity.`});
...
function TraderStats({s}){const rows=[['Trades',s.count],['Ganados / Perdidos',`${s.wins}W / ${s.losses}L`],['Sesgo más rentable',s.count?`${s.bestBias.name} · ${money(s.bestBias.value)}`:'—'],['Ganancia bruta',money(s.grossProfit)],['Pérdida bruta',money(s.grossLoss)],['Avg win',money(s.avgWin)],['Avg loss',money(s.avgLoss)],['Payoff ratio',s.payoffRatio?s.payoffRatio.toFixed(2):'0.00'],['Recovery factor',s.recovery?s.recovery.toFixed(2):'0.00'],['Media R',`${s.meanR.toFixed(2)}R`],['Volatilidad R',`${s.stdR.toFixed(2)}R`]];return <div className="statMatrix">{rows.map(([a,b])=><div key={a}><span>{a}</span><b>{b}</b></div>)}</div>}
...
function sanitizeFirestoreValue(value){
  ...
  const out={};
  Object.entries(value).forEach(([k,v])=>{const clean=sanitizeFirestoreValue(v); if(clean !== undefined) out[k]=clean;});
  return out;
}
function sanitizeFirestoreObject(obj={}){return sanitizeFirestoreValue(obj) || {};}
...
</button><button className="topCta primaryMini" onClick={()=>window.dispatchEvent(new CustomEvent('mtc-tab',{detail:'journal'}))}><Plus size={15}/> Registrar trade</button><span className="crownBadge" title="Moisés Trading Club"><Crown size={18}/></span><span className="cmdHint">Ctrl K</span></div></header>}
...
function DashboardControlCenter({profile,s,monthly,setTab,filtered=[]}){
  const dayTrades=filtered.filter(t=>(t.tradingDay||t.date||'').slice(0,10)===tradingDayKey());
...
function ChecklistStatusCard({data,setTab}){
  ...
  return <Card title="Checklist de Moisés" sub="Validador de Alta Probabilidad." className={`checklistDashCard ${state}`}>{last?<div className="checklistDash"><div><span>Última validación</span><b>{last.operationalState||last.status||'Sin estado'}</b><small>{last.asset||'—'} · Score {last.score||0}/100</small></div><div className={`lightOrb ${last.finalGreen?'green':'red'}`}>{last.finalGreen?'VERDE':'ROJA'}</div></div>:<div className="emptyPremium"><b>Todavía no validaste entradas.</b><p>Usá el Checklist de Moisés antes de operar.</p></div>}<div className="checklistStats checklistLinkedStats"><span>{aplus} setups A+ este mes</span><span>{cm.withChecklist.length} trades con checklist</span><span>{pct(cm.winrateGreen)} WR luz verde</span><span>{cm.aplus.length} A+ · {cm.plAPlus>=0?'+':''}{money(cm.plAPlus)}</span><button className="primary compact" onClick={()=>setTab('checklist')}>Nueva validación</button></div></Card>
}
function ActivityFeed({data,setTab}){
  ...
  const items=[
    ...
    ...(data.posts||[]).map(p=>({type:p.category||'Comunidad',text:p.body?.slice(0,70)||'Nueva publicación',meta:p.authorName||safeDate(p.createdAt)||'Alumno',target:'community',time:toTime(p.createdAt||p.date||p.createdDate)})),
  ...
  return <Card title="Actividad reciente" sub="Últimos movimientos del ecosistema."><div className="activityFeed">{items.map((it,i)=><button key={`${it.type}-${i}`} onClick={()=>setTab(it.target)}><span>{it.type}</span><b>{it.text}</b><small>{it.meta}</small></button>)}{!items.length&&<div className="emptyPremium"><b>Sin actividad reciente.</b><p>Cuando haya trades, checklists o publicaciones aparecerán acá.</p></div>}</div></Card>
}
function CalendarHeatmapPreview({trades=[]}){const stats=groupTradesByDay(trades); const key=monthKey(); const cells=daysInMonth(key); const monthStats=Object.values(stats).filter(x=>x.date.slice(0,7)===key); const total=monthStats.reduce((a,b)=>a+b.total,0); return <Card title="Calendario P/L" sub={`${monthStats.length} días operados · ${total>=0?'+':''}${money(total)}`} className="calendarPreviewCard"><div className="calendarMiniGrid">{cells.map((d,i)=>{if(!d)return <span key={i}/>; const st=stats[d]; const cls=!st?'none':st.total>0?'win':st.total<0?'loss':'be'; return <button key={d} className={cls} title={st?`${d} · ${money(st.total)}`:d}>{Number(d.slice(-2))}</button>})}</div></Card>}
...
function Journal({data,profile}){
  ...
  const guard=evaluateRiskGuard(data.trades,riskSettings,data.settings.initialBalance,dayPlan);
  const tradeDates=[...new Set((data.trades||[]).map(t=>(t.tradingDay||t.date||'').slice(0,10)).filter(Boolean))].sort((a,b)=>b.localeCompare(a));
...
function TradeDetailModal({trade,onClose,onEdit,onDelete,data}){
  ...
  return <div className="modal"><div className="modalCard tradeDetailModal"><div className="modalHead"><div><h3>{trade.asset} · {trade.side}</h3><p>{trade.date} · {trade.session} · {trade.tradeSystem||'Sistema de Moisés'}</p></div><div className="modalActions"><button className="ghost compact" onClick={onEdit}><Edit3 size={15}/>Editar</button>{onDelete&&<button className="ghost danger compact" onClick={onDelete}><Trash2 size={15}/>Eliminar</button>}<button onClick={onClose}><X size={20}/></button></div></div>
    ...
    {trade.mentorReviewRequested&&<div className={`mentorReviewStatusCard ${trade.mentorReviewStatus||'pending'}`}><span>Revisión del mentor</span><b>{mentorStatusLabel(trade.mentorReviewStatus||'pending')}</b><small>Foco: {trade.mentorReviewFocus||'general'}{trade.mentorReviewRequestedAt?` · solicitado ${String(trade.mentorReviewRequestedAt).slice(0,10)}`:''}</small>{trade.mentorReviewNote&&<p><b>Pregunta del alumno:</b> {trade.mentorReviewNote}</p>}{trade.mentorReviewResponse&&<p><b>Devolución:</b> {trade.mentorReviewResponse}</p>}</div>}
...
function TradeForm({form,setForm,profile,data}){
  ...
  async function save(){setBusy(true); console.info('saveTrade:start'); try{
    ...
    const reviewPatch=form.mentorReviewRequested?{
      ...
      mentorReviewRequestedBy:profile.uid,
      mentorReviewRequestedAt:form.mentorReviewRequestedAt||new Date().toISOString()
    }:{};
    const payload=numericTradePayload({...form,...reviewPatch,tradeSystem:form.tradeSystem||'Sistema de Moisés',system:form.tradeSystem||'Sistema de Moisés',setup:form.setup||normalizeTradeSetup(form)});
    const clean=sanitizeFirestoreObject({...payload,captureUrl,tradingDay:payload.tradingDay||payload.date||tradingDayKey(),date:payload.date||payload.tradingDay||tradingDayKey(),userId:profile.uid,uid:profile.uid,ownerId:profile.uid,updatedAt:serverTimestamp()});
    console.info('saveTrade:payload', clean);
    if(form.id){const {id,...rest}=clean; await setDoc(doc(db,'trades',form.id),rest,{merge:true}); console.info('saveTrade:success', form.id); toast('Trade actualizado correctamente');}
    else {const refTrade=await addDoc(collection(db,'trades'),{...clean,createdAt:serverTimestamp()}); if(clean.checklistId&&clean.createdFromChecklist){try{await setDoc(doc(db,'checklists',clean.checklistId),{linkedTradeId:refTrade.id,linkedTradeCreatedAt:serverTimestamp(),tradeDeleted:false},{merge:true});}catch(e){console.warn('No se pudo vincular checklist',e?.message)}} console.info('saveTrade:success', refTrade.id); toast(clean.createdFromChecklist?'Trade creado desde Checklist':'Trade guardado correctamente');}
    setForm(null);
  }catch(e){console.error('saveTrade:error',e); toast(e?.code==='permission-denied'?'No se pudo guardar el trade. Revisá conexión o permisos.':'No se pudo guardar el trade. Revisá conexión o datos.','error');}
  finally{setBusy(false)}}
  function captureFile(e){const f=e.target.files?.[0]; if(!f)return; setCaptureFileObj(f); ch('captureFileName',f.name)}
  ...
  if(quickFromChecklist){
    ...
    return <Card title="Trade rápido desde Checklist" sub="Completá resultado y comportamiento. Los datos técnicos ya vienen desde la validación." className="quickChecklistTradeForm">
      ...
      <div className="resultStrip resultHero editableResults">
        <div className={resultClass}><span>Profit / Loss en $</span><DecimalInput className="resultLiveInput" value={form.resultMoney} onChange={v=>ch('resultMoney',v)} placeholder="300" prefix="$"/><small>Dinero generado o perdido en este trade.</small></div>
        <div className={resultClass}><span>Profit / Loss en %</span><DecimalInput className="resultLiveInput" value={form.resultPct} onChange={v=>ch('resultPct',v)} placeholder="2.5" suffix="%"/><small>% hecho o perdido sobre la cuenta.</small></div>
  ...
  return <Card title="Nuevo trade" sub="Carga solo lo medible. Primero resultado en dinero, % y R; después contexto y checklist.">
    ...
    <div className="resultStrip resultHero editableResults">
      <div className={resultClass}><span>Profit / Loss en $</span><DecimalInput className="resultLiveInput" value={form.resultMoney} onChange={v=>ch('resultMoney',v)} placeholder="300" prefix="$"/><small>Dinero generado o perdido en este trade. Ej: -150.50</small></div>
      <div className={resultClass}><span>Profit / Loss en %</span><DecimalInput className="resultLiveInput" value={form.resultPct} onChange={v=>ch('resultPct',v)} placeholder="2.5" suffix="%"/><small>% hecho o perdido sobre la cuenta. Ej: -0.75</small></div>
    ...
    <div className="formGrid labeled">
      ...
      <Field label="Resultado" hint="Campo mínimo requerido para guardar."><select className="input" value={form.result||''} onChange={e=>ch('result',e.target.value)}><option value="">Seleccionar resultado...</option>{['Profit','Stop','BE','Invalidada','No ejecutada'].map(x=><option key={x}>{x}</option>)}</select></Field>
      <Field label="Cuenta / challenge"><select className="input" value={form.account||'Cuenta principal'} onChange={e=>ch('account',e.target.value)}>{normalizedAccounts(data?.settings||{}).map(a=><option key={a.name}>{a.name}</option>)}{!normalizedAccounts(data?.settings||{}).some(a=>a.name===(form.account||'Cuenta principal'))&&<option>{form.account||'Cuenta principal'}</option>}</select><small>Configura cuentas y capital desde Perfil.</small></Field>
      ...
      <Field label="Entry price" hint="Precio de entrada."><DecimalInput value={form.entry||''} onChange={v=>ch('entry',v)} placeholder="Ej: 2350.50"/></Field>
      ...
      <Field label="Exit price" hint="Precio real de salida."><DecimalInput value={form.exit||''} onChange={v=>ch('exit',v)} placeholder="Ej: 2357.50"/></Field>
...
function MentorReviewRequest({form,ch}){
  const requested=!!form.mentorReviewRequested;
  return <div className={`mentorReviewRequest ${requested?'on':''}`}><div><b>Revisión del mentor</b><p>Marcá este trade para que el mentor revise entrada, timing, gestión, contexto o psicología.</p></div><label className="switchLine"><input type="checkbox" checked={requested} onChange={e=>ch('mentorReviewRequested',e.target.checked)}/><span>Enviar este trade a revisión del mentor</span></label>{requested&&<div className="formGrid labeled"><Field label="¿Qué querés que revise?"><select className="input" value={form.mentorReviewFocus||'general'} onChange={e=>ch('mentorReviewFocus',e.target.value)}>{['general','entrada','timing','gestión','contexto','ejecución','psicología'].map(x=><option key={x} value={x}>{x[0].toUpperCase()+x.slice(1)}</option>)}</select></Field><Field label="Nota para el mentor"><TextareaWithEmoji className="input" value={form.mentorReviewNote||''} onChange={e=>ch('mentorReviewNote',e.target.value)} placeholder="Ej: quiero saber si entré tarde o si el contexto era válido."/></Field></div>}</div>
}
...
${msg}`))return;
  ...
  return <main className="page checklistPage"><section className={`checkHero ${ev.finalGreen?'ok':'bad'}`}><div><span className="pill gold"><CheckCircle2 size={14}/> Checklist de Moisés</span><h2>Validador de Alta Probabilidad</h2><p>Regla madre: checklist incompleto = no ejecutar. El score educa, pero la luz verde solo aparece si contexto, zona, RR y patrón están completos.</p></div><div className="checkScore"><b>{ev.score}</b><small>/100</small><span>{ev.quality}</span></div></section>
...
function ReadingPage({data,profile}){
  ...
  return <main className="page"><section className="heroSystem readingHero"><span className="pill gold">Biblioteca privada</span><h2>Biblioteca Moisés</h2><p>Todos los libros, PDFs y recursos quedan visibles en una biblioteca real para los alumnos. Podés filtrar, buscar, editar y borrar recursos.</p></section>
...
function RiskLab({data,profile}){
  ...
  const s=calc(data.trades,data.settings.initialBalance);
  const guard=evaluateRiskGuard(data.trades,settings,data.settings.initialBalance);
...
function Academy({data,profile}){
  ...
  async function saveLesson(){
    ...
    toast('Clase publicada en academia');
    setLesson(null);
  }
...
function Community({data,profile}){
  ...
  return <main className="page"><Card title="Comunidad privada" sub="Comparte resultados, dudas, aprendizajes y análisis. Todo queda ordenado dentro del club.">
    ...
    <div className="row"><select className="input small" value={cat} onChange={e=>setCat(e.target.value)}>{['Ideas de trading','Resultados','Backtesting','Dudas','Psicología','Gestión de riesgo','Setups Moisés','Comunidad','Anuncio'].map(x=><option key={x}>{x}</option>)}</select><button className="primary" onClick={add}><Plus/>Publicar</button></div>
  </Card>
  {editing&&<div className="modal"><div className="modalCard"><div className="modalHead"><h3>Editar publicación</h3><button onClick={()=>setEditing(null)}><X/></button></div><select className="input" value={editing.category} onChange={e=>setEditing({...editing,category:e.target.value})}>{['Ideas de trading','Resultados','Backtesting','Dudas','Psicología','Gestión de riesgo','Setups Moisés','Comunidad','Anuncio'].map(x=><option key={x}>{x}</option>)}</select><TextareaWithEmoji className="input" value={editing.body} onChange={e=>setEditing({...editing,body:e.target.value})}/><button className="primary full" onClick={saveEdit}>Guardar cambios</button></div></div>}
  <div className="feed">{[...data.posts].sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0)).map(p=><Post key={p.id} p={p} like={()=>like(p)} liked={(p.likes||[]).includes(profile.uid)} canEdit={admin||p.authorId===profile.uid} canPin={admin} onPin={()=>togglePin(p)} onEdit={()=>setEditing({id:p.id,body:p.body,category:p.category})} onDelete={()=>removePost(p)}/>)}</div></main>
}
...
function Results({data,profile}){
  ...
  return <main className="page resultsArena resultsPro"><section className="arenaHero upgraded"><div><span><Trophy size={16}/> Arena Moisés</span><h2>Resultados del Club</h2><p>Competí sanamente, compartí tu proceso y medí progreso con evidencia. Los resultados no se publican como casino: se publican con contexto, disciplina y aprendizaje.</p><div className="heroActions"><button className="primary compact" onClick={()=>document.querySelector('.resultComposer')?.scrollIntoView({behavior:'smooth'})}><Plus size={16}/> Subir resultado</button><button className="ghost compact" onClick={()=>{setFilter('season'); setTimeout(()=>document.getElementById('quarter-season')?.scrollIntoView({behavior:'smooth',block:'start'}),60)}}>Ver trimestre</button></div></div><div className="arenaScore"><b>{seasonId}</b><small>temporada activa</small><em>{seasonResults.length} resultados validados</em></div></section>
    ...
    <div id="quarter-ranking"><Card title="Podio de consistencia" sub="El podio solo considera resultados validados por mentor o destacados. Moisés Score: R, checklist, comportamiento, riesgo y participación."><div className="podiumGrid arenaPodium">{podium.map((r,i)=><div className={`podiumCard p${i+1}`} key={r.uid}><span>{i===0?'🥇':i===1?'🥈':'🥉'}</span><h3>{r.name}</h3><b>{r.score}/100</b><p>{classScore(r.score)} · {r.r.toFixed(2)}R · {Math.round(r.checklistPct)}% checklist · Behavior {Math.round(r.avgBehavior||0)}/100</p><div className="progress"><i style={{width:`${Math.min(100,r.score)}%`}}/></div></div>)}{!podium.length&&<Empty title="Sin ranking" text="Cuando haya resultados del trimestre aparecerá el podio."/>}</div></Card></div>
...
const coachKnowledge=[
 ...
 {keys:['stop','sl','stop loss'],ans:'El stop debe invalidar la idea, no proteger tu ego. Si movés el stop, convertís una pérdida pequeña en una decisión emocional.'},
...
function AccessGate({profile}){
  ...
  const statusCopy={
    ...
    expired:'Tu acceso venció. Reactivá tu membresía para seguir usando Moisés Trading Club.',
...
function HelpBot(){
  ...
  const waText=encodeURIComponent('Hola, soy alumno de Moisés Trading Club y necesito ayuda con la app.');
  ...
  const faqs=[
    ...
    ['📚 ¿Dónde veo los libros?','Entrá a Libros/Lectura. Ahí vas a ver la biblioteca del club, links a PDFs, descripciones y material recomendado.'],
    ...
    ['🔐 Mi cuenta está pendiente','Tu cuenta debe ser aprobada por un admin del club. Si ya sos alumno, escribí por WhatsApp para validar tu acceso.'],

...
const MOBILE_NAV_SHORT={dashboard:'Dash',journal:'Journal',checklist:'Check',ideas:'Ideas',risk:'Riesgo',analytics:'Analytics',results:'Results',brokers:'Broker',ecosystem:'Guía',emotional:'Emoción',system:'Sistema',reading:'Libros',news:'News',community:'Espacio',announcements:'Avisos',chat:'Chat',online:'Online',notifications:'Avisos',settings:'Perfil',admin:'Admin'};
function MobileNav({profile,tab,setTab,data}){const items=[['dashboard','Dashboard',Home],['journal','Journal',LineChart],['checklist','Checklist',CheckCircle2],['ideas','Ideas',Lightbulb],['risk','Riesgo',SlidersHorizontal],['analytics','Analytics',BarChart3],['results','Resultados',Trophy],['brokers','Integraciones',Activity],['ecosystem','Guía',Sparkles],['emotional','Emocional',Heart],['system','Sistema',Shield],['reading','Biblioteca',BookOpen],['news','Noticias',Newspaper],['community','Mi espacio',Users],['announcements','Anuncios',Megaphone],['chat','Chat',MessageCircle],['online','Online',Activity],['notifications','Avisos',Bell],['settings','Perfil',Settings]]; if(['admin','moderador'].includes(profile.role))items.push(['admin','Admin',Shield]); return <div className="mobileNav">{items.map(([id,label,Icon])=>{const count=activityCount(data,profile,id); const navLabel=MOBILE_NAV_SHORT[id]||label; return <button key={id} className={`${tab===id?'active':''} ${count?'hasActivity':''}`} onClick={()=>{markNotificationsForTarget(data.notifications,id); setTab(id);}}><Icon size={18}/><span>{navLabel}</span>{count>0&&<em>{count>9?'9+':count}</em>}</button>})}<button className="mobileLogout" onClick={()=>signOut(auth)}><LogOut size={18}/><span>Salir</span></button></div>}

...
function StructuredOnboardingCard({data,setTab}){
  ...
  const todayTrades=(data.trades||[]).filter(t=>(t.tradingDay||t.date||'').slice(0,10)===dayKey);
  ...
  const steps=[
    ...
    {label:'Guardar jornada del dia',done:journeyReady,target:'journal',cta:todayTrades.length?'Ver operativa':'Guardar jornada',icon:CalendarDays,openDate:dayKey},

...
function ProcessSummaryCard({data,setTab}){
  ...
  return <Card title="Resumen del proceso" sub="Evidencia registrada, sin metricas ficticias." className="processSummaryCard"><div className="processSummaryGrid">{rows.map(([label,value,target])=><button key={label} onClick={()=>setTab(target)}><span>{label}</span><b>{formatMetricCard(value)}</b></button>)}</div></Card>
}
...
function DashboardEmptyStates({data,setTab,s}){
  const items=[
    [!(data.trades||[]).length,'Todavia no hay trades registrados','Registra tu primera operacion para empezar a construir evidencia sobre tu proceso.','Registrar trade','journal',Plus],
    [!(data.checklists||[]).length,'Todavia no validaste entradas','Usa el checklist operativo antes de ejecutar para separar sistema de impulso.','Abrir checklist','checklist',CheckCircle2],
    [Number(s?.count||0)<5,'Analytics esperando evidencia','Carga operaciones para que las metricas empiecen a mostrar patrones reales.','Registrar trade','journal',BarChart3],
    [!(data.emotionalJournals||[]).length,'Sin revision emocional registrada','Cerra tu sesion para detectar como tu estado mental influye en la ejecucion.','Cerrar sesion emocional','emotional',Heart],
    [!(data.ideas||[]).length,'Sin ideas de trading cargadas','Guarda tus hipotesis para comparar plan, ejecucion y resultado.','Ver ideas','ideas',Lightbulb],
    [Number(s?.count||0)<10,'Performance en construccion','Cuando tengas mas registros, vas a poder leer consistencia, drawdown y calidad con mas precision.','Revisar analytics','analytics',Trophy]
  ].filter(([show])=>show);
...
function ChecklistStatusCard({data,setTab}){
  ...
  return <Card title="Checklist operativo" sub="Sistema de revisión antes de ejecutar." className={`checklistDashCard ${state}`}>{last?<div className="checklistDash"><div><span>Última validación</span><b>{last.operationalState||last.status||'Sin estado'}</b><small>{last.asset||'—'} · Score {last.score||0}/100</small></div><div className={`lightOrb ${last.finalGreen?'green':'red'}`}>{last.finalGreen?'VERDE':'ROJA'}</div></div>:<PremiumEmptyState title="Todavia no validaste entradas" text="Usa el checklist operativo antes de ejecutar." cta="Abrir checklist" icon={CheckCircle2} onClick={()=>setTab('checklist')}/>}<div className="checklistStats checklistLinkedStats"><span>{formatMetricCard(aplus)} setups A+ este mes</span><span>{formatMetricCard(cm.withChecklist.length)} trades con checklist</span><span>{pct(cm.winrateGreen)} WR luz verde</span><span>{formatMetricCard(cm.aplus.length)} A+ · {cm.plAPlus>=0?'+':''}{money(cm.plAPlus)}</span><button className="primary compact" onClick={()=>setTab('checklist')}>Nueva validación</button></div></Card>
}

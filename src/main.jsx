...
function StructuredOnboardingCard({data,setTab}){
  ...
  const steps=[
    ...
    {label:'Guardar jornada del dia',done:!!todayPlan,target:'journal',cta:'Guardar jornada',icon:CalendarDays,openDate:dayKey},
...
function SettingsPage({data,profile,setProfile}){
  ...
  function updateInitialBalance(value){const capital=Number(value); const accounts=normalizedAccounts(s); const nextAccounts=accounts.length?[{...accounts[0],capital},...accounts.slice(1)]:[{id:'main',name:'Cuenta principal',capital,type:'Personal',currency:'USD'}]; setS({...s,initialBalance:capital,accounts:nextAccounts})}
  async function saveAll(){const capital=Number(s.initialBalance||0); if(!Number.isFinite(capital)||capital<=0){toast('Ingresá un capital inicial mayor a 0.','error'); return;} await setDoc(doc(db,'users',profile.uid),{...u},{merge:true}); await setDoc(doc(db,'settings',profile.uid),s,{merge:true}); setProfile(u); toast('Cambios guardados')}

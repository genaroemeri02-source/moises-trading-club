...
function Shell({profile,tab,setTab,data,theme,toggleTheme}){const groups=[['OPERATIVA',[['dashboard','Dashboard',Home],['journal','Journal',LineChart],['emotional','Emocional',Heart],['brokers','Integraciones',Activity],['checklist','Checklist',CheckCircle2],['ideas','Ideas',Lightbulb],['risk','Riesgo',SlidersHorizontal]]],['SISTEMA',[['analytics','Analytics',BarChart3],['results','Resultados',Trophy]]],['WORKSPACE',[['academy','Base',BookOpen],['community','Workspace',Users],['announcements','Anuncios',Megaphone],['chat','Chat',MessageCircle],['online','Online',Activity]]],['RECURSOS',[['ecosystem','Guía',Sparkles],['system','Sistema',Shield],['reading','Setups',BookOpen],['news','Noticias',Newspaper]]],['PERFIL',[['notifications','Notificaciones',Bell],['settings','Perfil',Settings]]]]; if(['admin','moderador'].includes(profile.role))groups.push(['ADMIN',[['admin','Admin',Shield]]]); return <aside className="side premiumSide"><div className="brand"><img className="brandLogo" src="/brand/mtc-analytics-icon.png" alt="MTC Analytics"/><div><b>MTC Analytics</b><span>Trading performance platform</span></div></div><nav>{groups.map(([group,items])=><div className="navGroup" key={group}><small>{group}</small>{items.map(([id,label,Icon])=>{const count=activityCount(data,profile,id); return <button key={id} className={`${tab===id?'active':''} ${count?'hasActivity':''}`} onClick={()=>{markNotificationsForTarget(data.notifications,id); setTab(id);}}><Icon size={16}/><span>{label}</span>{count>0&&<em>{count>9?'9+':count}</em>}</button>})}</div>)}</nav><button
  onClick={toggleTheme}
  className="icon"
...
function ChecklistStatusCard({data,setTab}){
  ...
  return <Card title="Checklist operativo" sub="Sistema de revisión antes de ejecutar." className={`checklistDashCard ${state}`}>{last?<div className="checklistDash"><div><span>Última validación</span><b>{last.operationalState||last.status||'Sin estado'}</b><small>{last.asset||'—'} · Score {last.score||0}/100</small></div><div className={`lightOrb ${last.finalGreen?'green':'red'}`}>{last.finalGreen?'VERDE':'ROJA'}</div></div>:<div className="emptyPremium"><b>Todavía no validaste entradas.</b><p>Usá el checklist operativo antes de operar.</p></div>}<div className="checklistStats checklistLinkedStats"><span>{aplus} setups A+ este mes</span><span>{cm.withChecklist.length} trades con checklist</span><span>{pct(cm.winrateGreen)} WR luz verde</span><span>{cm.aplus.length} A+ · {cm.plAPlus>=0?'+':''}{money(cm.plAPlus)}</span><button className="primary compact" onClick={()=>setTab('checklist')}>Nueva validación</button></div></Card>
}
      ...
      <div className="paywallFunnelLayout">
        ...
        <aside className="paywallFunnelProof">
          ...
          <div className="paywallFunnelPreview">
            <video src="/paywall-product-demo.mp4" autoPlay muted loop playsInline preload="metadata" />
